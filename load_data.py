...
class SparseDataset(Dataset):
    ...
    def __init__(self, train_path, nfeatures):
        ...
        self.files = []
        self.file = [train_path +'/' + f for f in os.listdir(train_path)]
        self.dump_file = []
        for folder_path in self.file:
            ...
            self.dump_file += subfolders

        for folder_path in self.dump_file:
    ...
    def __getitem__(self, idx):
        ...
        kp1, descs1 = self.parse_superpoint_result(SuperPoint({'image': frame2tensor(image, device)}))
        kp2, descs2 = self.parse_superpoint_result(SuperPoint({'image': frame2tensor(warped, device)}))
