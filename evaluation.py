...
from dloc.core.utils.utils import (make_matching_plot, tensor_overlap_crop,
                                   vis_aligned_image)
from src.utils.utils import read_image, visualize_overlap_gt

from dloc.core.utils.base_model import dynamic_load
from dloc.core.utils.utils import (visualize_overlap,
...
class Matching(torch.nn.Module):
    ...
    def __init__(self, config=None, model_path=Path('D:/resource/ImageMatching-OETR/weights/')):
...
def viz_pairs(output,overlap_path,real0,real1, image0, image1, name0, name1, mconf, kpts0, kpts1,
              mkpts0, mkpts1):
    ...
    if not opt.show_keypoints :
        ...
        # Visualize the matches without keypoint.
        color = cm.jet(mconf)
        text = [
        'Matcher',
    ...
    overlap0 = overlap_path + name0
    overlap1 = overlap_path + name1
    matched_keys0 = (np.array(mkpts0, dtype=np.int32)).tolist()
    matched_keys1 = (np.array(mkpts0, dtype=np.int32)).tolist()
    matched_keys0 = np.array(matched_keys0).reshape(-1, 1, 2)
    matched_keys1 = np.array(matched_keys1).reshape(-1, 1, 2)

    cv2.polylines(real0, matched_keys0, True, (0, 255, 0), 2)
    cv2.polylines(real1, matched_keys1, True, (0, 0, 255), 2)

        # Tính toán bao lồi của các điểm feature matching
    hull1 = ConvexHull(matched_keys0)
    hull2 = ConvexHull(matched_keys1)
    
    #temp
    hull_indices1 = hull1.vertices
    hull_indices2 = hull2.vertices

    #set point 
    
    hull_pts1 = matched_keys0[hull_indices1, :][-1]
    print(hull_pts1)
    hull_pts2= matched_keys1[hull_indices2,: ][-1]
        # Tính toán diện tích bao lồi để có được diện tích chồng lấn
    overlap_area = cv2.contourArea(ConvexHull(np.vstack((matched_keys0, matched_keys1))))
    print(len(matched_keys0))
    if len(matched_keys0) >= 20 :
        # Vẽ hình ảnh vùng chồng lấn với màu khác nhau so với phần còn lại của hình ảnh gốc
        real0 = cv2.polylines(real0, [hull_pts1],True, (0, 255, 0),1)
        real1 = cv2.polylines(real1, [hull_pts2],True, (0, 255, 100),1)

    if opt.show_overlap :
        cv2.imshow('image', real0)
        # Hiển thị hình ảnh
    print(overlap_area)
    cv2.imwrite(overlap0,real0)
    cv2.imwrite(overlap1,real1)
...
def main(
    ...
    print(image_list)
    pair1 = os.listdir(os.path.join(input_dir,image_list[0]))
    pair2 = os.listdir(os.path.join(input_dir,image_list[1]))
    for i1, i2 in zip(pair1, pair2):
        pairs=[i1,i2]
        #pairs = list(combinations(pair1,pair2, 2))
        ...
        name0, name1 = pairs[0], pairs[1]
            ...
            #     continue
        gray = config['extractor']['preprocessing']
            # Load the image pair.
        align = ''
        ...
        if config['overlaper'] is not None:
                results = preprocess_overlap_pipeline(
                    os.path.join(input_dir,image_list[0]),
                    os.path.join(input_dir,image_list[1]),
                    name0,
                    name1,
                    device,
                    resize,
    ...
    if viz :
                ...
                if not os.path.exists(os.path.join(output, 'overlap')):
                    os.makedirs(os.path.join(output, 'overlap'))        
                
                viz_pairs(
                    os.path.join(output, 'viz/{}_'.format(scene.replace('/',
                                                                        '-'))),
                    os.path.join(output, 'overlap/{}_'.format(scene.replace('/',
                                                                        '-'))),
                    results['real_img0'],
                    results['real_img1'],                                              
                    results['image0'],
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--input_dir',
        type=str,
        default='D:\\resource\\ImageMatching-OETR\\dataset',
    ...
    parser.add_argument(
        '--show_overlap',
        action='store_true',
        help='Show_overlap area.',
    )
    parser.add_argument(
        '--show_keypoints',
        action='store_true',
        help='Not show keypoints',
    )
    parser.add_argument('--overlap_dir',
        type=str,
        default='dump_match_pairs/overlap',
        help='Overlap image saved directory')
